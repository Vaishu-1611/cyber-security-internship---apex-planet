Complete Task Portfolio (Task 1 – Task 5)
🚀 This repository contains all the tasks completed during my Cybersecurity & Ethical Hacking Internship at ApexPlanet.
The internship focused on building strong fundamentals, hands-on security testing, exploitation techniques, and real-world cybersecurity practices.

📌 Internship Overview

🏢 Organization: ApexPlanet Software Pvt. Ltd.

📅 Duration: 60 Days

🎯 Focus: Ethical Hacking, Penetration Testing & System Security

📂 Tasks Overview

🟢 Task 1: Foundations of Cybersecurity
📌 Objective
Build strong fundamentals in cybersecurity, networking, and cryptography while setting up a professional lab.

🟡 Task 2: Network Security & Scanning
📌 Objective
Understand reconnaissance, scanning, and network analysis.

🔵 Task 3: Web Application Security
📌 Objective
Identify and exploit OWASP Top 10 vulnerabilities.

🔴 Task 4: Exploitation & System Security
📌 Objective
Learn exploitation techniques and system defense strategies.

🟣 Task 5: Capstone Project & Incident Response
📌 Objective
Apply all learned skills in a real-world simulation project.

🧰 Tools & Technologies Used

Kali Linux

Metasploitable2

DVWA

Nmap

Wireshark

Burp Suite

Metasploit

Hydra

John the Ripper

OpenVAS / Nessus
